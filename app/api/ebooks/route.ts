  ...
  try {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .insert({
        ...
        type, drive_link: drive_link ?? null,
        price: price ?? null, cover_url: cover_url ?? null,
      })
      .select().single()
    if (error) throw error
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!id) return NextResponse.json({ error: 'ID required' }, { status: 400 })
    const { data: ebook } = await supabaseAdmin.from('ebooks').select('cover_url').eq('id', id).single()
    if (ebook?.cover_url) {
